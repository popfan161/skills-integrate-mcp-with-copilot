document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderActivities() {
    ...
    // Filter by category (simple heuristic based on name)
    const categoryValue = filterCategory ? filterCategory.value : "";
    if (categoryValue) {
      filtered = filtered.filter(([name]) => {
        if (categoryValue === "Club") return name.includes("Club");
        if (categoryValue === "Team") return name.includes("Team");
        if (categoryValue === "Class") return name.includes("Class");
        if (categoryValue === "Other") return !["Club","Team","Class"].some(cat => name.includes(cat));
        return true;
      });
    }
  ...
  // Render activity options for signup dropdown
